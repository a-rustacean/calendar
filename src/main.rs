use chrono::{offset::Local, Datelike, Duration, NaiveDate, Weekday};
...
fn main() {
    ...
    print!(
        ...
        repeat(
            " ",
            2 * from_sunday as usize + (from_sunday.max(1) as usize - 1)
        )
    );
    for day in 0..get_days_from_month(current_date.year(), current_date.month())
        .unwrap()
    {
